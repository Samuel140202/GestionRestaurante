document.getElementById('loginForm').addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch('http://localhost:3000/login', { // Asegúrate de que el puerto sea correcto
            method: 'POST',
            headers: {
        ...
        if (result.success) {
            ...
            window.location.href = 'inicio_admin.html'; // Redirigir a la página de éxito
        } else {
