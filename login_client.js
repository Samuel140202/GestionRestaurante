document.getElementById('loginForm').addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch('http://localhost:3000/login_client', { // Ruta específica para clientes
            method: 'POST',
            headers: {
        ...
        if (result.success) {
            ...
            window.location.href = 'inicio_cliente.html'; // Redirigir a la página de inicio de clientes
        } else {
